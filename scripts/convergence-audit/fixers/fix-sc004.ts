...
/**
 * Auto-fixer for SC-004: Replace ws:// with wss:// in WebSocket connections.
 */
...
function fixFile(filePath: string): { fixed: number; file: string } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/['"]ws:\/\//.test(lines[i])) {
      lines[i] = lines[i].replace(/(['"])ws:\/\//g, '$1wss://');
      fixed++;
    }
  }
...
function main(): void {
  ...
  console.log('');
  console.log(`SC-004: ${totalFixed} connexions ws:// corrigées dans ${filesChanged} fichiers.`);
}
