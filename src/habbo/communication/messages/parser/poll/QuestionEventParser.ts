 ...
 *
 * @see source_as/habbo/communication/messages/parser/poll/QuestionEventParser.as
 */
export class QuestionEventParser implements IMessageParser
{
	...
	parse(wrapper: IMessageDataWrapper): boolean
	{
		...
		if (questionType === 1 || questionType === 2)
		{
			...
			const selectionCount = wrapper.readInt();
			...
			question['selection_max'] = selectionCount;

			for (let i = 0; i < selectionCount; i++)
			{
				selectionValues.push(wrapper.readString());
				selections.push(wrapper.readString());
			}
		}
