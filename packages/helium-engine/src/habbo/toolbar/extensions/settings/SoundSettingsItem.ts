 ...
 * delegates volume saving to SoundSettingsView. In Helium, UI rendering is
 * handled by SolidJS.
 *
 * @see sources/win63_version/habbo/toolbar/extensions/settings/SoundSettingsItem.as
 */
export class SoundSettingsItem
{
	...
	private _parentView: SoundSettingsView | null;

	constructor(parentView: SoundSettingsView, type: number)
	{
		this._type = type;
		this._parentView = parentView;

		log.debug(`SoundSettingsItem constructed: type=${type}`);
	}
