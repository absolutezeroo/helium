 ...
 *
 * @see sources/flash_version/com/sulake/habbo/room/object/visualization/avatar/AvatarVisualization.as
 */
...
/** Maximum number of avatars with effect (scale factor for vertical offset). */
const MAX_AVATARS_WITH_EFFECT: number = 1000;

/** Sprite index for the main avatar composite image. */
const SPRITE_INDEX_AVATAR: number = 2;

/** Default canvas offsets when none are provided. */
const DEFAULT_CANVAS_OFFSETS: number[] = [0, 0, 0];

/** Sprite index for the shadow. */
const SPRITE_INDEX_SHADOW: number = 97;

/** Initial number of reserved sprites (avatar image + shadow + base). */
const INITIAL_RESERVED_SPRITES: number = 3;
...
{
	private _geometryUpdateId: number = -1000;
	...
	private _headAngle: number = 0;
	private _isAnimating: boolean = false;
	private _figure: string = '';
	private _gender: string = '';
	private _currentAngleDeg: number = 0;
	...
	override initialize(data: IRoomObjectVisualizationData): boolean
	{
		...
		this.createSprites(SPRITE_INDEX_AVATAR);
	...
	override update(geometry: IRoomGeometry, time: number, update: boolean, skipUpdate: boolean): void
	{
		...
		if (time < (this._geometryUpdateId + GEOMETRY_UPDATE_INTERVAL_MS))
		{
			return;
		}

		this._geometryUpdateId = this._geometryUpdateId + GEOMETRY_UPDATE_INTERVAL_MS;

		if ((this._geometryUpdateId + GEOMETRY_UPDATE_INTERVAL_MS) < time)
		{
			this._geometryUpdateId = time - GEOMETRY_UPDATE_INTERVAL_MS;
		}
	...
	private updateModel(model: IRoomObjectModel, scale: number, isFullUpdate: boolean): boolean
	{
		...
		if (model.getUpdateID() !== this._updateModelCounter)
		{
			...
			// Vertical offset
			numValue = model.getNumber(RoomObjectVariableEnum.AVATAR_VERTICAL_OFFSET) * MAX_AVATARS_WITH_EFFECT;
	...
	private getAvatarImageForScale(scale: number, effectType: number): IAvatarImage | null
	{
		...
		if (image == null)
		{
			...
			if (image != null)
			{
				...
				{
					if (this._avatarImagesWithEffect.size >= INITIAL_RESERVED_SPRITES)
					{
	...
	{
		const shadowSprite = this.getSprite(1);
	...
	private updateActions(avatarImage: IAvatarImage): void
	{
		...
		let spriteCount = SPRITE_INDEX_AVATAR;
	...
	private updateExtraSprites(offsets: number[]): void
	{
		...
		let spriteIndex = SPRITE_INDEX_AVATAR;
		...
		for (const spriteData of this._activeAvatarImage.getSprites())
		{
			...
			else
			{
				...
				if (sprite != null)
				{
					...
					{
						sprite.offsetY = sprite.offsetY + ((this._verticalOffset * scale) / (2 * MAX_AVATARS_WITH_EFFECT));
					}
					else
					{
