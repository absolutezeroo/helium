...
import type { AnimationAction } from './structure/animation/AnimationAction';
...
export class AvatarStructure
{
            ...
            if(animation)
            {
                ...
                if(animation.hasAddData())
                {
                    for(const addData of animation.addData)
                    {
                        ...
                        if(bodyPart)
                        {
                            bodyPart.addPart({ id: addData.id, x: 0, y: 0, z: 0, radius: 0.01, nx: 0, ny: 0, nz: -1, double: 1 }, avatar);

                            const partDef = this._partSetsData.addPartDefinition({ 'set-type': addData.id, setType: addData.id });
    ...
    public getFrameBodyPartOffset(action: IActiveActionData, direction: number, frameIndex: number, bodyPartId: string): { x: number; y: number }
    {
