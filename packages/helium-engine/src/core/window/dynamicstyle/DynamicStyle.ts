...
export class DynamicStyle
{
     ...
     */
    public getColorTransform(state: number): { redMultiplier: number; greenMultiplier: number; blueMultiplier: number; alphaMultiplier: number; redOffset: number; greenOffset: number; blueOffset: number; alphaOffset: number }
    {
        const props = this.getStyleByWindowState(state);
        const ct = props.colorTransform as number[] | undefined;
