...
export class DropBaseController extends InteractiveController
{
	...
	)
	{
		...
		if (region)
		{
			(region as unknown as IWindow).addEventListener('WME_DOWN', this._menuItemEventHandler.bind(this));
		}
	}
	...
	public override dispose(): void
	{
		...
		if (region)
		{
			(region as unknown as IWindow).removeEventListener('WME_DOWN', this._menuItemEventHandler.bind(this));
		}
		...
		if (!this._menuIsOpen)
		{
			if (this.open())
			{
				...
				if (this.numMenuItems > 0)
				{
					...
					this.populateExpandedMenu(this._itemArray, this.getExpandedMenuView(), this._subMenuEventProc.bind(this) as (event: unknown, window: IWindow) => void);
				}
			}
		}
	}
	...
	private getExpandedMenuView(): DropBaseController
	{
		...
		if (this._subMenu === null || this._subMenu.disposed)
		{
			...
			this._subMenu = this._context.create(
				...
				paramFlags,
				rect,
				this._subMenuEventProc.bind(this) as (event: unknown, window: IWindow) => void,
				null,
				0,
				null,
				''
